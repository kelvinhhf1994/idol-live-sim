...
import type { HouseLights, VenueBuild } from "./createVenue";
...
export function createNgauTauKokVenue(definition: VenueDefinition): VenueBuild {
  ...
  const mats: SharedMaterials = {
    ...
    // Faint emissive stands in for bounced light in the bright foyer/mall (three.js has no local ambient)
  ...
  const crowdDummies = createAtmosphericCrowd(group);
...
function aimAngles(from: THREE.Vector3, to: THREE.Vector3): { pan: number; tilt: number } {
...
function createBeamCones(beams: readonly FakeBeam[]): THREE.InstancedMesh {
...
// ---------------------------------------------------------------------------
function createExitSignTexture(): THREE.Texture {
  if (typeof document === "undefined") return new THREE.Texture();
  const canvas = document.createElement("canvas");
  canvas.width = 512;
  canvas.height = 200;
...
// ---------------------------------------------------------------------------
// Crowd with penlights (ported)
// ---------------------------------------------------------------------------
function createAtmosphericCrowd(group: THREE.Group): THREE.Group[] {
  ...
  const crowdPositions: [number, number][] = [
...
function woodPlankTexture(): THREE.Texture {
  if (typeof document === "undefined") return new THREE.Texture();
  const canvas = document.createElement("canvas");
  canvas.width = 1024;
  canvas.height = 1024;
  ...
  const plankColors = ["#c49a68", "#cfa672", "#d6b07c", "#bd9260", "#b48a5a", "#c9a06c"];
  let seed = 7;
  const rand = () => {
  ...
  for (let y = 0; y < 1024; y += plankHeight) {
    ...
    while (x < 1024) {
      ...
      ctx.fillStyle = plankColors[Math.floor(rand() * plankColors.length)] ?? "#c49a68";
      ctx.fillRect(x, y, plankWidth, plankHeight);

      // Grain: many faint wavy lines
      ctx.strokeStyle = "rgba(90, 55, 25, 0.16)";
      ctx.lineWidth = 1;
      for (let g = 0; g < 6; g++) {
...
function pleatedCurtainTexture(): THREE.Texture {
  if (typeof document === "undefined") return new THREE.Texture();
  const canvas = document.createElement("canvas");
  canvas.width = 512;
  canvas.height = 512;
...
function speakerGrilleTexture(): THREE.Texture {
  if (typeof document === "undefined") return new THREE.Texture();
  const canvas = document.createElement("canvas");
  canvas.width = 256;
  canvas.height = 256;
...
function consoleMixerTexture(): THREE.Texture {
  if (typeof document === "undefined") return new THREE.Texture();
  const canvas = document.createElement("canvas");
  canvas.width = 512;
  canvas.height = 256;
...
function createFridgeTexture(): THREE.Texture {
  if (typeof document === "undefined") return new THREE.Texture();
  const canvas = document.createElement("canvas");
  canvas.width = 512;
  canvas.height = 512;
